...
export async function generateMetadata({ params }) {
  ...
  return generateMetadatas({
    ...
    url: `https://www.mediaeyenews.com/category/msme/${categorySlug}`,
    img: imageUrl,
  });
}

export default async function MSMECategoryPage({ params }) {
  const { categorySlug, page } = await params;

  console.log("MSME Category Slug:", categorySlug);
  ...
  try {
    // Get category ID by categorySlug (technical-training)
      ...
      return (
        <>
          <CategoryPostList
            ...
            posts={[]}
            page_link_url={`/category/msme/${categorySlug}/`}
            perPage={perPage}
            totalPage={0}
